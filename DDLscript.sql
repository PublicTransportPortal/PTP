...
CREATE INDEX IFK_Rel_04 ON usersearch (portalUser_idportalUser);
GO
...
CREATE INDEX IFK_Rel_05 ON feedback (usersearch_idusersearch);
GO


CREATE TABLE usersearch_has_Bus (
  usersearch_idusersearch INTEGER  NOT NULL  ,
  Bus_idBus CHAR(15)  NOT NULL    ,
...
CREATE INDEX IFK_Rel_05 ON usersearch_has_Bus (usersearch_idusersearch);
GO
CREATE INDEX IFK_Rel_06 ON usersearch_has_Bus (Bus_idBus);
GO
...
PRIMARY KEY(Bus_idBus, Point_idPoint)    ,
  FOREIGN KEY(Bus_idBus)
    REFERENCES Bus(idBus),
...
CREATE INDEX IFK_Rel_03 ON Bus_has_Point (Bus_idBus);
GO
CREATE INDEX IFK_Rel_04 ON Bus_has_Point (Point_idPoint);
GO
